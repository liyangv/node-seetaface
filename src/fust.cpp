...
//  std::ifstream model_file(model_path, std::ifstream::binary);
    ...
    std::istringstream model_file(std::string(g_classifierBuffer, g_classifierBuffer + sizeof(g_classifierBuffer)),std::ifstream::binary);//,
  bool is_loaded = true;

  if (false) { //!model_file.is_open()
    is_loaded = false;
  } else {
